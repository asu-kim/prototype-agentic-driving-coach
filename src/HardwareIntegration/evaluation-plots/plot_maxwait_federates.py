...
RELAXED_MAXWAIT_TRACE_DIR = REPO_ROOT / "src/HardwareIntegration/evaluation/logs-trace-relaxed-our-assumption"
TIGHT_MAXWAIT_TRACE_DIR = REPO_ROOT / "src/HardwareIntegration/evaluation/logs-trace-tight-maxwait"
OUT_DIR = REPO_ROOT / "src/HardwareIntegration/evaluation-plots/out/maxwait-federates"
...
PLOT_FONT_SIZE = 30
RELAXED_MAXWAIT_COLOR = "#1e3a8a"
TIGHT_MAXWAIT_COLOR = "#ea580c"
TRACE_LABELS = {
    "relaxed": "Relaxed maxwait",
    "tight": "Tight maxwait",
}
...
def plot_bar(
    ...
    colors = [RELAXED_MAXWAIT_COLOR, TIGHT_MAXWAIT_COLOR]
    all_values = []
...
def write_summary(out_dir: Path, rows: list[dict[str, object]]) -> None:
    out_dir.mkdir(parents=True, exist_ok=True)
    path = out_dir / "maxwait_federate_metrics.csv"
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Plot tardy handler invocation counts and p95 lag by maxwait setting."
    )
    parser.add_argument("--relaxed-trace-dir", type=Path, default=RELAXED_MAXWAIT_TRACE_DIR)
    parser.add_argument("--tight-trace-dir", type=Path, default=TIGHT_MAXWAIT_TRACE_DIR)
    parser.add_argument("--out-dir", type=Path, default=OUT_DIR)
    args = parser.parse_args()

    apply_large_bold_style()
    relaxed_df = read_execution_logs(args.relaxed_trace_dir)
    tight_df = read_execution_logs(args.tight_trace_dir)
    available_feds = set(relaxed_df["federate"].astype(str)) | set(tight_df["federate"].astype(str))
    feds = [fed for fed in FED_ORDER if fed in available_feds]
    labels = [FED_LABELS[fed] for fed in feds]
    relaxed_tardy_counts = [tardy_count_for(relaxed_df, fed) for fed in feds]
    tight_tardy_counts = [tardy_count_for(tight_df, fed) for fed in feds]
    relaxed_lag_p95 = [p95_lag_for(relaxed_df, fed) for fed in feds]
    tight_lag_p95 = [p95_lag_for(tight_df, fed) for fed in feds]

    rows = [
        {
            "condition": TRACE_LABELS["relaxed"],
            "federate": fed,
            "label": FED_LABELS[fed],
            "tardy_handler_invocations": tardy,
            "lag_p95_ms": lag,
        }
        for fed, tardy, lag in zip(feds, relaxed_tardy_counts, relaxed_lag_p95)
    ]
    rows.extend(
        {
            "condition": TRACE_LABELS["tight"],
            "federate": fed,
            "label": FED_LABELS[fed],
            "tardy_handler_invocations": tardy,
            "lag_p95_ms": lag,
        }
        for fed, tardy, lag in zip(feds, tight_tardy_counts, tight_lag_p95)
    )
    ...
    plot_bar(
        labels,
        {
            TRACE_LABELS["relaxed"]: relaxed_tardy_counts,
            TRACE_LABELS["tight"]: tight_tardy_counts,
        },
        "Number of tardy handler invocations",
        args.out_dir,
        "maxwait_tardy_handler_invocations_by_federate",
    )
    plot_bar(
        labels,
        {
            TRACE_LABELS["relaxed"]: relaxed_lag_p95,
            TRACE_LABELS["tight"]: tight_lag_p95,
        },
        "p95 lag (ms)",
        args.out_dir,
        "maxwait_lag_p95_by_federate",
    )
