...
TRACE_DIR = REPO_ROOT / "src/HardwareIntegration/evaluation/logs-trace-normal-assumption-network"
NETWORK_DELAY_TRACE_DIR = REPO_ROOT / "src/HardwareIntegration/evaluation/logs-trace-added-network-delay"
OUT_DIR = REPO_ROOT / "src/HardwareIntegration/evaluation-plots/out/normal-assumption-federates"
...
PLOT_FONT_SIZE = 30
BAR_COLOR = "#2563eb"
NETWORK_DELAY_COLOR = "#f97316"
TRACE_LABELS = {
    "nominal": "Nominal network latency",
    "additional_network_latency": "Additional network latency of 200ms",
}
...
def plot_bar(
    ...
    colors = [BAR_COLOR, NETWORK_DELAY_COLOR]
    all_values = []
...
def write_summary(out_dir: Path, rows: list[dict[str, object]]) -> None:
    out_dir.mkdir(parents=True, exist_ok=True)
    path = out_dir / "normal_assumption_federate_metrics.csv"
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Plot tardy handler invocation counts and p95 lag by HardwareADC federate."
    )
    parser.add_argument("--trace-dir", type=Path, default=TRACE_DIR)
    parser.add_argument("--network-delay-trace-dir", type=Path, default=NETWORK_DELAY_TRACE_DIR)
    parser.add_argument("--out-dir", type=Path, default=OUT_DIR)
    args = parser.parse_args()

    apply_large_bold_style()
    nominal_df = read_execution_logs(args.trace_dir)
    network_delay_df = read_execution_logs(args.network_delay_trace_dir)
    available_feds = set(nominal_df["federate"].astype(str)) | set(network_delay_df["federate"].astype(str))
    feds = [fed for fed in FED_ORDER if fed in available_feds]
    labels = [FED_LABELS[fed] for fed in feds]
    nominal_tardy_counts = [tardy_count_for(nominal_df, fed) for fed in feds]
    network_delay_tardy_counts = [tardy_count_for(network_delay_df, fed) for fed in feds]
    nominal_lag_p95 = [p95_lag_for(nominal_df, fed) for fed in feds]
    network_delay_lag_p95 = [p95_lag_for(network_delay_df, fed) for fed in feds]

    rows = [
        {
            "condition": TRACE_LABELS["nominal"],
            "federate": fed,
            "label": FED_LABELS[fed],
            "tardy_handler_invocations": tardy,
            "lag_p95_ms": lag,
        }
        for fed, tardy, lag in zip(feds, nominal_tardy_counts, nominal_lag_p95)
    ]
    rows.extend(
        {
            "condition": TRACE_LABELS["additional_network_latency"],
            "federate": fed,
            "label": FED_LABELS[fed],
            "tardy_handler_invocations": tardy,
            "lag_p95_ms": lag,
        }
        for fed, tardy, lag in zip(feds, network_delay_tardy_counts, network_delay_lag_p95)
    )
    ...
    plot_bar(
        labels,
        {
            TRACE_LABELS["nominal"]: nominal_tardy_counts,
            TRACE_LABELS["additional_network_latency"]: network_delay_tardy_counts,
        },
        "Number of tardy handler invocations",
        args.out_dir,
        "tardy_handler_invocations_by_federate",
    )
    plot_bar(
        labels,
        {
            TRACE_LABELS["nominal"]: nominal_lag_p95,
            TRACE_LABELS["additional_network_latency"]: network_delay_lag_p95,
        },
        "p95 lag (ms)",
        args.out_dir,
        "lag_p95_by_federate",
